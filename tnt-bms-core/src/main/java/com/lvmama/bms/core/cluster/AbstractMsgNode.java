...
import com.lvmama.bms.core.cmd.JVMInfoGetHttpCmd;
import com.lvmama.bms.core.cmd.StatusCheckHttpCmd;
...
import com.lvmama.bms.core.compiler.AbstractCompiler;
import com.lvmama.bms.core.constant.EcTopic;
import com.lvmama.bms.core.constant.ExtConfig;
import com.lvmama.bms.core.factory.MsgNodeConfigFactory;
import com.lvmama.bms.core.factory.NodeFactory;
import com.lvmama.bms.core.failstore.FailStorePathBuilder;
import com.lvmama.bms.core.json.JSONFactory;
import com.lvmama.bms.core.listener.MasterChangeListener;
import com.lvmama.bms.core.listener.MasterElectionListener;
import com.lvmama.bms.core.listener.NodeChangeListener;
import com.lvmama.bms.core.listener.SelfChangeListener;
...
import com.lvmama.bms.core.registry.*;
import com.lvmama.bms.core.spi.ServiceLoader;
import com.lvmama.bms.core.support.AliveKeeping;
import com.lvmama.bms.core.support.ConfigValidator;
 ...
 *         抽象节点
 */
public abstract class AbstractMsgNode<T extends Node, Context extends AppContext> implements MsgNode {

    protected static final Logger LOGGER = LoggerFactory.getLogger(MsgNode.class);
    ...
    public AbstractMsgNode() {
        appContext = getAppContext();
        node = NodeFactory.create(getNodeClass());
        config = MsgNodeConfigFactory.getDefaultConfig();
    ...
    final public void start() {
        try {
            ...
            if (e.getMessage().contains("Address already in use")) {
                LOGGER.error("Start=failed|listen_port={}|nodeType={}|identity={}", config.getListenPort(), config.getNodeType(), config.getIdentity(), e);
            } else {
    ...
    protected void initConfig() {
        ...
        if (StringUtils.isEmpty(config.getIdentity())) {
            MsgNodeConfigFactory.buildIdentity(config);
        }
...
//        registry = RegistryFactory.getRegistry(appContext);
                    ...
                    case ADD:
                        for (NodeChangeListener listener : nodeChangeListeners) {
                            try {
                                ...
                                NOTIFY_LOGGER.error("tip={} add nodes failed , cause: {}", listener.getClass().getName(), t.getMessage(), t);
                            }
                        }
                        break;
                    case REMOVE:
                        for (NodeChangeListener listener : nodeChangeListeners) {
                            try {
                                ...
                                NOTIFY_LOGGER.error("tip={} remove nodes failed , cause: {}", listener.getClass().getName(), t.getMessage(), t);
                            }
                        }
                        break;
                }
    ...
    protected abstract void beforeRemotingStart() throws Exception;
