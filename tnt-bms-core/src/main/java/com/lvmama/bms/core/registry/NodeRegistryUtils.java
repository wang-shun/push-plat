...
public class NodeRegistryUtils {
        ...
        try {
            ...
            for (String paramEntry : paramArr) {
                ...
                if ("clusterName".equals(key)) {
                    ...
                    node.setGroup(value);
                } else if ("fastThreads".equals(key)) {
                    node.setFastThreads(Integer.valueOf(value));
                } else if ("slowThreads".equals(key)) {
                    node.setSlowThreads(Integer.valueOf(value));
    ...
    public static String getFullPath(Node node) {
        ...
        if (node.getFastThreads() != 0) {
            path.append("&fastThreads=")
                    .append(node.getFastThreads());
        }

        if(node.getSlowThreads() !=0) {
            path.append("&slowThreads=")
                    .append(node.getSlowThreads());
        }
